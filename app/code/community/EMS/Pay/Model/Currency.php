...
class EMS_Pay_Model_Currency
{
    ...
    /**
     * @param string|Mage_Directory_Model_Currency $currency code or object
     * @return int
     * @throws Exception
     */
    public function getNumericCurrencyCode($currency)
    {
        $code = $currency instanceof Mage_Directory_Model_Currency ?
            $currency->getCode() :
            (string)$currency;
    ...
    public function getCurrencyLabel($currencyCode)
    {
        ...
        if (isset($this->_currencyLabels[$currencyCode])) {
            $label = $this->_helper->__($this->_currencyLabels[$currencyCode]);
        }
